...
public class Grafo {
	...
	private NodoVert eliminarArcoAux(NodoVert nodoOrigen, Object destino) {
		// Método que elimina un arco dado el vértice y el adyacente, devuelve la referencia del vértice del arco eliminado
		...
		if(adyacente != null) {
			if(destino.equals(adyacente.getVertice().getElem())) {
				// Si el primer adyacente es el que buscamos ponemos el primerAdy del origen como el siguiente
				...
				while(encontrado == null && adyacente.getSigAdyacente() != null) {
					if(destino.equals(adyacente.getSigAdyacente().getVertice().getElem())) {
						// encontrado es el vértice del arco (por si se quiere borrar también de ese lado)
